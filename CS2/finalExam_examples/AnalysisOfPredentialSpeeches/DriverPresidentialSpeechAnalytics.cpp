...
using namespace std;
/*
This program analyzes presidential speeches to see how
frequently president used certain
words in his speeches. Press reporters sometime
get an idea about presidential thoughts,
...
int main()
{
	...
	while (in.peek() != EOF)
	{
		string foo1;
		in >> foo1;
		// Make sure that word only contains alphabets.
		string foo;

		for (size_t i = 0; i < foo1.length(); i++)
		{
			unsigned char ch = static_cast<unsigned char>(foo1[i]);
			if (isalpha(ch))
			{
				foo += ch;
			}
