...
bool LinearSearch(const Student arr[], int logicalLen, const Student & key, 
	int & whereFound, int flag);
int main()
{
	Student StuArr[MAX];
	//	Student(string frst, string lst, int idNum, int ag, double gp)
	...
	print(StuArr, 4);
	//Search based on ID or last name
	bool done = false;

	while (!done)
	{
		cout << "1. Search based on ID: " << endl
			<< "2. Search based on last name (case sensitive):" << endl
			<< "3. Exit search:" << endl;
		int choice;
		cin >> choice;
		if (choice == 1)
		{
			cout << "Enter student ID being searched. [whole number only]:";
			int idNum;
			cin >> idNum;
			//	Student(string frst, string lst, int idNum, int ag, double gp)
			Student Key("", "", idNum, 0, 0.0);
			int index;
			bool found = LinearSearch(StuArr, 4, Key, index, 1);

			if (found)
			{
				cout << "Student found. Full record is below." << endl;
				StuArr[index].print();
			}
			else
			{
				cout << "Student with ID: " << idNum << " is not found." << endl;
			}
		}
		else if (choice == 2)
		{
			// complete yourself based on model shown in menu item 1
		}
		else
		{
			done = true;
...
/*
LinearSearch returns true if key is found in the array arr.
Function also updates the value of index whereFound to a value
>=0 but <logicalLen. That would be the index in the array where key is found.
If key is not in the array, then function returns false
and sets whereFound to a value of -1
*/
bool LinearSearch(const Student arr[], int logicalLen, 
	const Student & key, int & whereFound, int flag)
{
	bool found = false;
	whereFound = -1;
	/*
	flag = 1 search based on ID
	flag = 2: Search based on last name
	*/
	for (int i = 0; i < logicalLen; i++)
	{
		if (flag == 1)// search based on ID
		{
			if (key.ID == arr[i].ID)
			{
				whereFound = i;
		...
		else if (flag == 2)// search based on last name - case sensitive
		{
			if (key.last == arr[i].last)
			{
				whereFound = i;
				found = true;
				break;
