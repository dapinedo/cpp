...
int main()
{
	...
	int lenQ = fillQuestionsArray(in, "questions"); // Filling the questions array
	...
	int lenA = fillAnswerKeyArray(in, "answers"); // Filling the answer key array
...
void printAnswersAndResponses(const string questions[], const char answers[], const char response[], int len)
{
	...
	for (size_t i = 0; i < len; i++)
	{
		...
		cout << "Your Answer = " << response[i] << emndl;
		if (answers[i] == response[i])
		{
